...
//    }
    ...
    if (callbacks & INTERRUPT_CALLBACK) {
        fmt_ctx->interrupt_callback.callback = interruptCallback;
        fmt_ctx->interrupt_callback.opaque = fmt_ctx;
    }
    if (!(avio_ctx = avio_alloc_context(avio_buffer, BUFFER_SIZE, write_flag, fmt_ctx, reader, writer, seeker))) {
        av_free(avio_buffer);
        avformat_free_context(fmt_ctx);
        return AVERROR(ENOMEM);
    }
...
AVFrame *convert_frame_to_rgb(AVFrame *frame, int alpha) {
    ...
    sws_ctx = sws_getContext(frame->width, frame->height, frame->format, output_frame->width, output_frame->height,
                             output_fmt, SWS_LANCZOS | SWS_ACCURATE_RND, NULL, NULL, NULL);
    if (!sws_ctx) {
        goto free;
    }
